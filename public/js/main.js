...
if (null !== txtarea) {
    txtarea.addEventListener('keydown', function (e) {
        ...
        document.querySelector('form').onsubmit = function() {
            document.getElementById('content').value = document.getElementById('seo-editor').innerHTML;
        };
        ...
        function insertLink() {
            ...
            const rel = prompt("Enter rel attribute (e.g., 'nofollow'):");
            if (url) {
                const linkHTML = `<a href="${url}" title="${title}" rel="${rel}" target="_blank">${url}</a>`;
        ...
        function insertImage() {
            ...
            const altText = prompt("Enter the image description:");
            if (url) {
        ...
        function showWordCount() {
            const text = document.getElementById('seo-editor').innerText;
...
// Initialize Quill editor
        ...
        document.querySelector('form').onsubmit = function() {
            document.getElementById('content').value = quill.root.innerHTML; // Set content to the hidden input
        };
        ...
        quill.getModule('toolbar').addHandler('image', () => {
            ...
            input.onchange = () => {
                ...
                reader.onload = (e) => {
                    ...
                    const width = prompt("Enter width (e.g., 200px or 50%):", "200px");
                    const height = prompt("Enter height (e.g., 200px or auto):", "auto");
                    insertImageToEditor(imageDataUrl, altText, alignment, width, height);
                };
                reader.readAsDataURL(file);
            };
        });

        // Insert Image with Alt Text, Alignment, and Dimensions
        function insertImageToEditor(imageUrl, altText, alignment, width, height) {
            const range = quill.getSelection();
            const floatStyle = alignment === 'left' ? 'float: left; margin-right: 10px;' :
                               alignment === 'right' ? 'float: right; margin-left: 10px;' :
                               'display: block; margin: 0 auto;'; // center alignment

            quill.insertEmbed(range.index, 'image', imageUrl);
            quill.insertText(range.index + 1, altText, { 'alt': altText });
            const imgHTML = `<img src="${imageUrl}" alt="${altText}" style="${floatStyle} width: ${width}; height: ${height};" />`;
            quill.root.innerHTML += imgHTML; // Add image to the content
        }